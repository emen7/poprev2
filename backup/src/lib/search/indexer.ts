 ...
 */
export async function buildSearchIndex(documents: Array<(TransformedDocument & { id: string; path: string; type?: string }) | null>): Promise<SearchableDocument[]> {
  // Filter out null documents
  const validDocuments = documents.filter((doc): doc is TransformedDocument & { id: string; path: string; type?: string } => doc !== null);
 ...
 */
function determineDocumentType(doc: TransformedDocument & { id: string; path: string }): 'scientific' | 'perplexity' | 'lectionary' | 'post' {
  // Check path first
 ...
 */
function extractMetadata(doc: TransformedDocument & { id: string; path: string }): Record<string, any> {
  const metadata = { ...doc.metadata };
