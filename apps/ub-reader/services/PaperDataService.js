...
function getPaperTitle(paperNumber) {
...
function getPaperAuthor(paperNumber) {
...
function getSectionTitles(paperNumber) {
 ...
 */
export async function getPaper(paperNumber) {
    try {
    ...
    return {
        ...
        sections: sectionTitles.map((title, index) => ({
            ...
            paragraphs: Array.from({ length: 2 }, (_, i) => ({
                number: i + 1,
                text: `This is paragraph ${i + 1} of section ${index + 1} in paper ${paperNumber}. In a real implementation, this would be the actual text from the Urantia Book.`,
            })),
        })),
    };
}
 ...
 */
export function getPartForPaper(paperNumber) {
    if (paperNumber >= 1 && paperNumber <= 31)
        return 1;
    if (paperNumber >= 32 && paperNumber <= 56)
        return 2;
    if (paperNumber >= 57 && paperNumber <= 119)
        return 3;
    if (paperNumber >= 120 && paperNumber <= 196)
        return 4;
 ...
 */
export function getPreviousPaper(paperNumber) {
 ...
 */
export function getNextPaper(paperNumber) {
    // Handle special cases
    if (paperNumber >= 196)
        return null;
    return paperNumber + 1;
...
//# sourceMappingURL=PaperDataService.js.map