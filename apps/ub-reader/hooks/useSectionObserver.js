...
sectionHeadingSelector = '.section-content', containerSelector = '.content', // Changed from .reading-area to .content
additionalOffset = 50, // Further increased offset for better detection
    ...
    const lastVisibleSectionRef = useRef('(intro)');
    // Debug state
    ...
    useEffect(() => {
        ...
        const row3Height = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--row3-height') || '2rem');
        ...
        if (process.env.NODE_ENV === 'development') {
            console.log('Hot zone position:', hotZonePosition);
        ...
        if (isIntersectionObserverSupported()) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('Section observer setup with rootMargin:', options.rootMargin);
                console.log('Total header height:', row1Height + row2Height + row3Height);
            }
            // Create the observer
            observerRef.current = new IntersectionObserver(entries => {
                ...
                if (debug) {
                    console.log('Observer triggered with visible sections:', visibleSections.length);
                    console.log('All entries:', entries.length);
                    // Advanced debug logging
                    visibleSections.forEach(entry => {
                        var _a;
                        console.log(`Section "${(_a = entry.target.textContent) === null || _a === void 0 ? void 0 : _a.substring(0, 30)}..." - intersectionRatio: ${entry.intersectionRatio.toFixed(2)}, top: ${entry.boundingClientRect.top.toFixed(0)}px`);
                    });
                    // Log all entries in debug mode
                    if (entries.length > 0 && visibleSections.length === 0) {
                        console.log('No visible sections. First entry:', (_a = entries[0].target.textContent) === null || _a === void 0 ? void 0 : _a.substring(0, 30));
                        console.log('First entry top:', entries[0].boundingClientRect.top);
                        console.log('Hot zone position:', hotZonePosition);
                    }
                }
                ...
                const sortedSections = [...visibleSections].sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top);
                ...
                if (titleElement && titleElement.textContent) {
                    ...
                    if (debug) {
                        console.log('Setting current section to:', sectionTitle);
                    }
            ...
            sections.forEach(section => {
                var _a;
                (_a = observerRef.current) === null || _a === void 0 ? void 0 : _a.observe(section);
            });
        }
        // Method 2: Fallback to scroll event listener
            ...
            const detectActiveSection = () => {
                ...
                else if (scrollTop < 100 // Only reset to intro at the very top
                ) {
        ...
        return () => {
            if (observerRef.current) {
                sections.forEach(section => {
                    var _a;
                    (_a = observerRef.current) === null || _a === void 0 ? void 0 : _a.unobserve(section);
                });
            }
        };
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            if (e.ctrlKey && e.shiftKey && e.altKey && e.key === 'S') {
                console.log('Toggling section observer debug mode');
                setDebug(prev => !prev);
