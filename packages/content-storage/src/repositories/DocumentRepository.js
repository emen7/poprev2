...
export class DocumentRepository extends BaseRepository {
     ...
     */
    async getByPublicationId(publicationId) {
     ...
     * @returns Promise resolving to an array of documents
     */
    async getByPublicationIdAndType(publicationId, type) {
     ...
     */
    async getByPublicationIdAndNumber(publicationId, number) {
        const results = await this.find(doc => doc.publicationId === publicationId && doc.number === number);
     ...
     */
    async getSectionById(documentId, sectionId) {
     ...
     * @returns Promise resolving to the section or null if not found
     */
    async getSectionByNumber(documentId, sectionNumber) {
        const document = await this.getById(documentId);
        if (!document) {
            return null;
        }
        const section = document.sections.find((s) => s.number === sectionNumber);
        return section || null;
    }
    /**
     ...
     */
    async getParagraphById(documentId, sectionId, paragraphId) {
     ...
     * @returns Promise resolving to the paragraph or null if not found
     */
    async getParagraphByNumber(documentId, sectionId, paragraphNumber) {
        const section = await this.getSectionById(documentId, sectionId);
        if (!section) {
            return null;
        }
     ...
     */
    async addSection(documentId, section) {
        const document = await this.getById(documentId);
        if (!document) {
            throw new Error(`Document with ID ${documentId} not found`);
        }
        // Check if section with same ID already exists
        const existingSection = document.sections.find((s) => s.id === section.id);
        if (existingSection) {
     ...
     * @returns Promise resolving to the updated document
     */
    async updateSection(documentId, sectionId, updates) {
        const document = await this.getById(documentId);
        if (!document) {
            throw new Error(`Document with ID ${documentId} not found`);
        }
        ...
        updatedSections[sectionIndex] = Object.assign(Object.assign({}, updatedSections[sectionIndex]), updates);
        return this.update(documentId, { sections: updatedSections });
    }
    /**
     ...
     * @returns Promise resolving to the updated document
     */
    async removeSection(documentId, sectionId) {
        const document = await this.getById(documentId);
        if (!document) {
            throw new Error(`Document with ID ${documentId} not found`);
        }
     ...
     * @returns Promise resolving to the updated document
     */
    async addParagraph(documentId, sectionId, paragraph) {
        const document = await this.getById(documentId);
        if (!document) {
            throw new Error(`Document with ID ${documentId} not found`);
        }
        ...
        const existingParagraph = document.sections[sectionIndex].paragraphs.find((p) => p.id === paragraph.id);
        if (existingParagraph) {
            throw new Error(`Paragraph with ID ${paragraph.id} already exists in section ${sectionId}`);
        }
        // Add the paragraph
        const updatedSection = Object.assign(Object.assign({}, document.sections[sectionIndex]), { paragraphs: [...document.sections[sectionIndex].paragraphs, paragraph] });
        // Sort paragraphs by number
        updatedSection.paragraphs.sort((a, b) => {
            const aNum = typeof a.number === 'number' ? a.number : parseInt(a.number, 10);
            const bNum = typeof b.number === 'number' ? b.number : parseInt(b.number, 10);
            return aNum - bNum;
        });
     ...
     * @returns Promise resolving to the updated document
     */
    async updateParagraph(documentId, sectionId, paragraphId, updates) {
        const document = await this.getById(documentId);
        if (!document) {
            throw new Error(`Document with ID ${documentId} not found`);
        }
        ...
        // Find the paragraph
        const paragraphIndex = document.sections[sectionIndex].paragraphs.findIndex((p) => p.id === paragraphId);
        if (paragraphIndex === -1) {
        ...
        updatedParagraphs[paragraphIndex] = Object.assign(Object.assign({}, updatedParagraphs[paragraphIndex]), updates);
        // Update the section
        const updatedSections = [...document.sections];
        updatedSections[sectionIndex] = Object.assign(Object.assign({}, updatedSections[sectionIndex]), { paragraphs: updatedParagraphs });
        return this.update(documentId, { sections: updatedSections });
    }
    /**
     ...
     * @returns Promise resolving to the updated document
     */
    async removeParagraph(documentId, sectionId, paragraphId) {
        const document = await this.getById(documentId);
        if (!document) {
            throw new Error(`Document with ID ${documentId} not found`);
        }
        ...
        const updatedParagraphs = document.sections[sectionIndex].paragraphs.filter((p) => p.id !== paragraphId);
