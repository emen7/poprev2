 ...
 */
export const HighlightProvider = ({ children, containerSelector, isDarkMode, colors, onHighlight, showHighlights: initialShowHighlights = true, currentPaper, currentSection, }) => {
        ...
        if (currentPaper && highlightManager) {
            ...
            if (storedHighlights) {
                try {
                    ...
                    const paperHighlights = parsedHighlights.filter(h => h.paperNumber === currentPaper &&
                    ...
                    paperHighlights.forEach(highlight => {
                        ...
                        if (paragraphElement) {
                            ...
                            const textNode = Array.from(paragraphElement.querySelectorAll('.ub-paragraph-text')).find(node => { var _a; return (_a = node.textContent) === null || _a === void 0 ? void 0 : _a.includes(highlight.text); });
                            if (textNode) {
    ...
    useEffect(() => {
        if (highlights.length > 0 && currentPaper) {
            ...
            const storageHighlights = highlights.map(highlight => {
                ...
                const paragraphElement = (_a = highlight.range.startContainer.parentElement) === null || _a === void 0 ? void 0 : _a.closest('.ub-paragraph');
            ...
            if (storedHighlights) {
                try {
                    ...
                    allHighlights = allHighlights.filter(h => h.paperNumber !== currentPaper ||
                        (currentSection && h.sectionNumber !== currentSection));
                }
                catch (error) {
                    console.error('Error parsing stored highlights:', error);
    ...
    return (_jsx(HighlightContext.Provider, { value: {
            highlightManager,
            showHighlights,
            setShowHighlights,
            highlights,
