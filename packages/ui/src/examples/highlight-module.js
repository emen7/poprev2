...
class HighlightManager {
   ...
   * @param {HTMLElement} options.container - The container element where highlighting will be applied
   * @param {Array} options.colors - Array of color objects with name, lightModeColor, and darkModeColor properties
   * @param {Function} options.isDarkMode - Function that returns true if dark mode is active
   * @param {Function} options.onHighlight - Callback function when text is highlighted
   * @param {boolean} options.showHighlights - Whether to show highlights (default: true)
   */
  constructor(options) {
   ...
   * @returns {Array} Array of default color objects
   */
  getDefaultColors() {
    return [
   ...
   * @param {Selection} selection - The selection to process
   * @returns {string} The plain text of the selection
   */
  processSelectionForCopy(selection) {
  ...
  handleClickOutside(event) {
    ...
    if (
      this.colorPicker &&
      !this.colorPicker.contains(event.target) &&
      !this.container.contains(event.target)
    ) {
      this.removeColorPicker();
    }
  ...
  /**
   * Create selection menu
   * @param {Object} position - The position of the menu
   */
  ...
  /**
   * Show color picker
   * @param {Object} position - The position of the color picker
   */
  ...
  applyHighlight(colorName) {
    ...
    try {
      ...
      alert('Could not apply highlight: ' + error.message);
    }
  }

  /**
   * Process contents for highlighting
   * @param {DocumentFragment} contents - The contents to process
   * @param {string} className - The class name to apply
   */
  processContentsForHighlighting(contents, className) {
    ...
    textNodes.forEach(textNode => {
      ...
      if (parent && parent.classList && parent.classList.contains('ub-highlight')) {
   ...
   * @param {Range} range - The range to remove highlights from
   */
   ...
   * @param {boolean} show - Whether to show highlights
   */
...
// Export the HighlightManager class
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { HighlightManager };
} else {
  window.UBHighlight = { HighlightManager };
}
