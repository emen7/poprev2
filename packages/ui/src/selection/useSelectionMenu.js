 ...
 */
export function useSelectionMenu({ targetElement = null, showOnRightClick = true, showOnMouseUp = true, minSelectionLength = 1, offset = { top: 10, left: 0 }, darkMode = false, } = {}) {
    ...
    const showMenu = useCallback((position) => {
        const selection = window.getSelection();
        if (!selection || selection.isCollapsed)
            return;
        const selectionText = selection.toString().trim();
        if (selectionText.length < minSelectionLength)
            return;
    ...
    const highlightSelectedText = useCallback((color) => {
        const selection = window.getSelection();
        if (!selection || selection.isCollapsed)
            return;
