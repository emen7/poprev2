 ...
 * - Visibility tracking
 */
export function ContentRenderer({ content, formatType, showParagraphNumbers = true, highlightedSections = [], highlightedParagraphs = [], onSectionVisible, onParagraphVisible, className = '', }) {
    const contentRef = useRef(null);
    // Set up intersection observer for tracking visible sections
    useEffect(() => {
        if (!contentRef.current || !onSectionVisible)
            return;
        const options = {
            root: null,
    ...
    useEffect(() => {
        if (!contentRef.current || !onParagraphVisible)
            return;
        const options = {
            root: null,
    ...
    const contentClasses = [
        ...
        .filter(Boolean)
        .join(' ');
    return (_jsxs("div", { className: contentClasses, ref: contentRef, children: [_jsx("h1", { className: "content-title", children: content.title }), content.sections.map(section => (_jsx(SectionRenderer, { section: section, formatType: formatType, showParagraphNumbers: showParagraphNumbers, isHighlighted: highlightedSections.includes(section.id), highlightedParagraphs: highlightedParagraphs, onVisible: onSectionVisible, onParagraphVisible: onParagraphVisible, onParagraphClick: (paragraphId, event) => {
