...
export function NavigationExample() {
  ...
  const filteredDocument = Object.assign(Object.assign({}, mockDocument), {
    sections: currentSection ? [currentSection] : [],
  });
  // Create header content
  const headerLeftContent = _jsx(NavigationControls, {
  ...
  const headerCenterContent = _jsx('h1', {
    className: 'reader-title',
    children: mockDocument.title,
  });
  const headerRightContent = _jsxs('div', {
    className: 'reader-controls',
    children: [
      _jsx('button', {
        onClick: toggleFormat,
        className: 'reader-control-button',
        children: formatType === 'traditional' ? 'Traditional' : 'Modern',
      }),
      _jsx('button', {
        onClick: toggleNumbers,
        className: 'reader-control-button',
        children: showNumbers ? 'Hide Numbers' : 'Show Numbers',
      }),
    ],
  });
  // Create footer content
  const footerLeftContent = _jsxs('div', {
    className: 'reader-section-info',
    children: ['Section ', currentSectionIndex + 1, ' of ', mockDocument.sections.length],
  });
  const footerRightContent = _jsx(NavigationControls, {
    prevEnabled: hasPrevious,
    nextEnabled: hasNext,
    onPrevious: handlePrevious,
    onNext: handleNext,
      ...
      _jsxs('div', {
        ...
        children: [
          ...
          _jsx('main', {
            ...
            children: _jsx(ContentContainer, {
              width: 'medium',
              centered: true,
              padding: 'normal',
