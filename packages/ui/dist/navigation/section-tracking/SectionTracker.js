 ...
 */
export function SectionTracker({ sectionId, sectionTitle, children, debug = false, className = '', }) {
    const { setCurrentSection, updateSectionTitle } = useNavigation();
    const lastUpdateTimeRef = useRef(0);
    // Debounce section updates to prevent rapid changes
    const debouncedUpdateSection = useCallback((entry) => {
        const now = Date.now();
    ...
    const sectionClasses = [
        ...
        .filter(Boolean)
        .join(' ');
    return (_jsx("div", { ref: ref, className: sectionClasses, "data-section-id": sectionId, id: sectionId, children: children }));
}
