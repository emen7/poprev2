import { PhoneticFormat, PronunciationEntry } from '../types';
 ...
 */
export function tagContentWithPronunciation(
  content: string, 
  format: PhoneticFormat = 'ssml'
): string {
  ...
  const sortedTerms = [...urantiaPronunciations].sort(
  ...
  return sortedTerms.reduce((taggedContent, entry) => {
    ...
    if (format === 'ssml') {
      ...
      return taggedContent.replace(termRegex, 
 ...
 * @returns The tagged content
 */
export function tagSpecificTerms(
  content: string,
  terms: string[],
  format: PhoneticFormat = 'ssml'
): string {
  ...
  const entries = terms
    .map(term => urantiaPronunciations.find(
      entry => entry.term.toLowerCase() === term.toLowerCase()
    ))
    .filter((entry): entry is PronunciationEntry => entry !== undefined);
 ...
 */
export function extractUrantiaTerms(content: string): string[] {
 ...
 */
export function addPauses(content: string): string {
