 ...
 */
function parsePerplexityFormat(content: string): any {
  try {
 ...
 */
function extractPerplexityMetadata(parsedContent: any): DocumentMetadata {
  return {
 ...
 */
function convertToInternalFormat(parsedContent: any): RootNode {
 ...
 */
function generateHtml(content: RootNode): string {
  // Simple HTML generation for preview purposes
  let html = '';

  function processNode(node: DocumentNode): string {
    switch (node.type) {
      case 'root':
      ...
      case 'link':
        return `<a href="${(node as any).url}" title="${(node as any).title || ''}">${node.children ? node.children.map(processNode).join('') : ''}</a>`;
