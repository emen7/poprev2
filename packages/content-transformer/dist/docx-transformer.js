 ...
 */
export async function transformDocx(content) {
 ...
 */
async function extractDocxMetadata(content) {
    try {
        ...
        if (lines.length > 0) {
            ...
            const authorLine = lines.find(line => line.includes('By:') ||
                line.includes('Author:') ||
                line.match(/^By\s+[A-Z]/));
            if (authorLine) {
                const authorMatch = authorLine.match(/(?:By:|Author:)\s*(.+)/) ||
                    authorLine.match(/^By\s+(.+)/);
 ...
 */
function convertHtmlToInternalFormat(html) {
    ...
    root.children = [
        ...
        ...headings.map(h => ({
            ...
            children: [
                {
                    type: 'text',
                    value: h.content.replace(new RegExp(`<h${h.level}>(.*?)<\/h${h.level}>`), '$1')
                        .replace(/<[^>]*>/g, '') // Remove any HTML tags
                }
            ]
        }))
...
//# sourceMappingURL=docx-transformer.js.map