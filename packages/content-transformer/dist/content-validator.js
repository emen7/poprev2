 ...
 */
function validateContentStructure(rootNode) {
...
function sanitizeContent(rootNode) {
    ...
    const sanitizedRoot = Object.assign(Object.assign({}, rootNode), { children: [] });
...
function sanitizeNode(node) {
    ...
    const sanitizedNode = Object.assign({}, node);
...
function validateMetadata(metadata) {
    ...
    const validatedMetadata = Object.assign({}, metadata);
    ...
    if (validatedMetadata.author) {
        if (Array.isArray(validatedMetadata.author)) {
            validatedMetadata.author = validatedMetadata.author.map((author) => typeof author === 'string' ? author : String(author));
        }
    ...
    if (validatedMetadata.categories) {
        if (!Array.isArray(validatedMetadata.categories)) {
            ...
            validatedMetadata.categories = validatedMetadata.categories.map((category) => typeof category === 'string' ? category : String(category));
        }
    }
    ...
    if (validatedMetadata.tags) {
        if (!Array.isArray(validatedMetadata.tags)) {
            ...
            validatedMetadata.tags = validatedMetadata.tags.map((tag) => typeof tag === 'string' ? tag : String(tag));
        }
    }
